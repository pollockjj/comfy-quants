"""export-model-w8a8 command (INT8 W8A8 + optional ConvRot, for ComfyUI-INT8-Fast)."""
...
from comfy_quants.formats.int8_w8a8 import INT8_W8A8_FORMAT_NAME
...
_DEFAULT_CHECKPOINT_NAME = f"diffusion_pytorch_model.{INT8_W8A8_FORMAT_NAME}.safetensors"


def register(subparsers):
    parser = subparsers.add_parser(
        "export-model-w8a8",
        help="Export a full INT8 W8A8 (+ConvRot) inference checkpoint for ComfyUI-INT8-Fast",
    )
    ...
    parser.add_argument("--convrot", action=argparse.BooleanOptionalAction, default=True, help="Apply ConvRot (regular-Hadamard) weight rotation before INT8 quantization")
...
def _build_tensor_index(cfg, source: Path) -> dict:
    ...
    # INT8 W8A8 is per-output-channel regardless of the config scale section.
...
def run(args) -> int:
    ...
    if cfg.quant.target_dtype != INT8_W8A8_FORMAT_NAME:
        raise ConfigurationError(
            f"export-model-w8a8 requires quant.target_dtype '{INT8_W8A8_FORMAT_NAME}', got {cfg.quant.target_dtype}"
        )
