"""export-model-mxfp8 command (MXFP8 microscaling FP8, for stock ComfyUI)."""
...
from comfy_quants.formats.mxfp8 import MXFP8_FORMAT_NAME
...
_DEFAULT_CHECKPOINT_NAME = f"diffusion_pytorch_model.{MXFP8_FORMAT_NAME}.safetensors"


def register(subparsers):
    parser = subparsers.add_parser(
        "export-model-mxfp8",
        help="Export a full MXFP8 block-scaled inference checkpoint for stock ComfyUI (Blackwell)",
    )
...
def _build_tensor_index(cfg, source: Path) -> dict:
    ...
    # MXFP8 is block-32 along the input dim, with an E8M0 scale; fixed by the format.
...
def run(args) -> int:
    ...
    if cfg.quant.target_dtype != MXFP8_FORMAT_NAME:
        raise ConfigurationError(
            f"export-model-mxfp8 requires quant.target_dtype '{MXFP8_FORMAT_NAME}', got {cfg.quant.target_dtype}"
        )
