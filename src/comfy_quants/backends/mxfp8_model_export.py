"""Safetensors MXFP8 (OCP microscaling FP8) inference-checkpoint exporter.

OFFLINE producer of **stock-ComfyUI-native** MXFP8 checkpoints. Mirrors the FP8
full-checkpoint writer (:mod:`comfy_quants.backends.inference_model_export`) but
stores ``torch.float8_e4m3fn`` weights with a per-32-element **E8M0** block scale in
the cuBLAS ``to_blocked`` swizzle layout (``uint8`` on disk), plus a per-layer
``comfy_quant`` marker. Emits NO ``input_scale`` (activations are quantized
dynamically at runtime).

Loaded by stock ComfyUI via ``QUANT_ALGOS["mxfp8"]`` + ``TensorCoreMXFP8Layout``
(``comfy/ops.py`` ``_load_from_state_dict`` for the ``mxfp8`` branch). The mxfp8
tensor-core matmul is Blackwell-gated; on other hardware ComfyUI silently
dequantizes the weight to the compute dtype (loads & correct, no speedup).

Block math + the ``to_blocked`` swizzle live in
:mod:`comfy_quants.formats.mxfp8_blocked` (pure torch, bit-faithful to
``comfy/float.py``).
"""
...
# resolution, config copy, metadata, progress). Importing them keeps the
# load-bearing FP8 writer untouched and avoids duplication (same pattern as the
# INT8 W8A8 writer).
...
from comfy_quants.formats.mxfp8 import MXFP8_FORMAT_NAME, mxfp8_checkpoint_quant_config
from comfy_quants.formats.mxfp8_blocked import BLOCK_SIZE, quantize_mxfp8_block
from comfy_quants.utils.hashing import hash_file


@dataclass
class MxFp8CheckpointExportReport:
    """Summary of a full MXFP8 checkpoint export."""
    ...
    schema_version: str = "mxfp8_checkpoint_export_report.v1"
    ...
    target_dtype: str = MXFP8_FORMAT_NAME
    quant_storage_dtype: str = "float8_e4m3fn"
    scale_dtype: str = "float8_e8m0fnu"
...
def _resolve_mxfp8_target_dtype(tensor_index: dict[str, Any], target_dtype: str | None = None) -> str:
    index_dtype = tensor_index.get("format", {}).get("name")
    resolved = target_dtype or index_dtype
    if resolved != MXFP8_FORMAT_NAME:
        raise PayloadWriteError(f"tensor index is not an {MXFP8_FORMAT_NAME} format: {resolved}")
    if index_dtype != MXFP8_FORMAT_NAME:
        raise PayloadWriteError(f"tensor index format {index_dtype} does not match requested target dtype {resolved}")
    return MXFP8_FORMAT_NAME
...
def _check_selected_rows(tensor_index: dict[str, Any], *, target_dtype: str) -> dict[str, dict[str, Any]]:
    ...
    for row in list(tensor_index.get("tensors") or []):
        if row.get("quant_dtype") != target_dtype:
            raise PayloadWriteError(f"unsupported quant dtype for MXFP8 export: {row.get('quant_dtype')}")
        if row.get("storage_dtype") != "uint8":
            raise PayloadWriteError(f"unsupported storage dtype for MXFP8 export: {row.get('storage_dtype')}")
...
def write_mxfp8_inference_checkpoint_from_safetensors(
    *,
...
) -> MxFp8CheckpointExportReport:
    """Write a full checkpoint with selected weights stored as MXFP8 tensors."""
    safe_open, save_file = _require_safetensors()
    torch = _require_torch()
    resolved_target_dtype = _resolve_mxfp8_target_dtype(tensor_index, target_dtype)
    ...
    for source_file_index, (source_file, tensor_names) in enumerate(source_files, start=1):
        ...
        with safe_open(str(source_file), framework="pt", device="cpu") as handle:
            ...
            for tensor_name in tensor_names:
                ...
                qweight, weight_scale = quantize_mxfp8_block(tensor_for_quant)
                ...
                output_tensors[f"{layer}.comfy_quant"] = _marker_tensor(mxfp8_checkpoint_quant_config(), device="cpu")
                dtype_counts["float8_e4m3fn"] = dtype_counts.get("float8_e4m3fn", 0) + 1
                dtype_counts["uint8"] = dtype_counts.get("uint8", 0) + 2  # weight_scale + comfy_quant
                quantized += 1
                _emit_progress(
                    progress,
    ...
    output_metadata.update(
        {
            ...
            "target_dtype": resolved_target_dtype,
            "quant_storage_dtype": "float8_e4m3fn",
    ...
    written_files = [
        {
            ...
            "kind": "mxfp8_inference_checkpoint",
            "state": "written",
    ...
    return MxFp8CheckpointExportReport(
