"""MXFP8 (OCP microscaling FP8) quantization planner.

Like ``fp8_static``, the planner only assigns per-module quantize/keep actions and
stamps the target dtype; the block-scale/swizzle math lives in the backend writer
(``backends/mxfp8_model_export.py``) and ``formats/mxfp8_blocked.py``.
"""
...
registry.register_algorithm(MxFp8Algorithm())
