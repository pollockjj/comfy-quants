"""Static Qwen-Image model contract."""
...
CONTRACT_SCHEMA_VERSION = "qwen_image_static_contract.v1"
...
def _block_modules(component: str, block_prefix: str) -> tuple[ModuleContract, ...]:
    ...
    return (
        ...
        _norm_module(f"{prefix}.img_norm1", component, module_type="LayerNorm", notes="image-stream normalization kept high precision"),
        _norm_module(f"{prefix}.img_norm2", component, module_type="LayerNorm", notes="image-stream normalization kept high precision"),
        ...
        _norm_module(f"{prefix}.txt_norm1", component, module_type="LayerNorm", notes="text-stream normalization kept high precision"),
        _norm_module(f"{prefix}.txt_norm2", component, module_type="LayerNorm", notes="text-stream normalization kept high precision"),
...
def _post_modules(component: str) -> tuple[ModuleContract, ...]:
    return (
        ...
        _norm_module("norm_out.norm", component, module_type="LayerNorm", notes="final normalization kept high precision"),
        _linear_module(
            "proj_out",
            component,
...
def _extra_components(include_visual_semantic_path: bool) -> tuple[ModuleContract, ...]:
    components: list[ModuleContract] = [
    ...
    if include_visual_semantic_path:
        components.append(
            _extra_component(
                "visual_semantic_path",
                "ImageEditConditioningPath",
                "visual_semantic_path",
                "image-edit conditioning path kept high precision",
            )
        )
    return tuple(components)


def build_qwen_image_static_contract(
    *,
    family: str,
    schema_version: str,
    export_name: str,
    include_visual_semantic_path: bool = False,
    ...
    return QwenModelContract(
        ...
        extra_components=_extra_components(include_visual_semantic_path),
        metadata=contract_metadata,
    )


def get_qwen_image_static_contract() -> QwenModelContract:
    return build_qwen_image_static_contract(
        family="qwen_image",
        schema_version=CONTRACT_SCHEMA_VERSION,
        export_name="Qwen-Image",
        include_visual_semantic_path=False,
        metadata={
            "supported_model_ids": ("Qwen/Qwen-Image", "Qwen/Qwen-Image-2512"),
            "calibration_profile": "text_to_image",
        },
    )
